...
## finding the unique character lengths for each variable
lengths <- c(19,39,59)
...
# POSIX conversion needs to happen here, not in explore
...
splitDF <- splitDF %>% mutate(`morning end` = as.POSIXct(`morning end`))

## PM
splitDF <- splitDF %>% mutate(`afternoon start` = as.POSIXct(`afternoon start`))
splitDF <- splitDF %>% mutate(`afternoon end` = as.POSIXct(`afternoon end`))